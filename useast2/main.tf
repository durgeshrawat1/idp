...
locals {
  ...
  kms_key_arn   = var.kms_key_arn != "" ? var.kms_key_arn : "arn:aws:kms:us-east-2:${data.aws_caller_identity.current.account_id}:key/default-key"
}
...
data "aws_vpc" "selected" {
  tags = {
    Name = var.useast2.selected_vpc_name
  }
}
...
data "aws_subnets" "selected" {
  ...
  filter {
    name   = "tag:Name"
    values = var.useast2.selected_subnet_names
  }
}
...
module "pattern1" {
  ...
  enabled = var.useast2.enable_pattern1
